...
type CreateBlogReducerTypes = (typeof CreateBlogReducerType)[keyof typeof CreateBlogReducerType];
...
export function createBlogReducer(state: IBlog, action: CreateBlogReducerAction): IBlog {
    switch (action.type) {
...
export function overrideBlog(blog: IBlog): CreateBlogReducerAction {
    return { type: CreateBlogReducerType.OVERRIDE_BLOG, payload: JSON.stringify(blog) }
}