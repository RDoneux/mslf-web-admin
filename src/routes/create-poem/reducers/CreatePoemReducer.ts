...
type CreatePoemReducerTypes =
  (typeof CreatePoemReducerType)[keyof typeof CreatePoemReducerType];
...
export function createPoemReducer(
  state: IPoem,
  action: CreatePoemReducerAction
): IPoem {
  switch (action.type) {
...
export function overridePoem(poem: IPoem): CreatePoemReducerAction {
  return {
    type: CreatePoemReducerType.OVERRIDE_POEM,
    payload: JSON.stringify(poem)
  };
}
